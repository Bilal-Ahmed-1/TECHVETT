    ...
    <div class="mb-4 d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
      <div>
        <h2 class="fw-bold mb-1 text-accent">
          <i class="fas fa-network-wired me-2"></i> NETWORKING CANDIDATES
        </h2>
        ...
        <table class="table table-dark table-hover table-bordered align-middle text-center mb-0">
          ...
          <tbody>
            <tr v-for="(candidate, index) in filteredCandidates" :key="candidate.id">
              ...
              <td>
                <button class="btn btn-sm btn-outline-accent" @click="openForm(candidate, index)">Edit</button>
                <button class="btn btn-sm btn-outline-danger" @click="deleteCandidate(candidate.id)">Delete</button>
              </td>
            </tr>
...
export default {
  ...
  methods: {
    async fetchCandidates() {
      try {
        const response = await axios.get('/candidate-networking');
    ...
    async saveCandidate() {
      try {
        if (this.form.id) {
          await axios.put(`/candidate-networking/${this.form.id}`, this.form);
        } else {
          await axios.post('/candidate-networking', this.form);
        }
        this.fetchCandidates();
        this.closeForm();
      } catch (error) {
    ...
    async deleteCandidate(id) {
      if (confirm('Are you sure you want to delete this candidate?')) {
        try {
          await axios.delete(`/candidate-networking/${id}`);
          this.fetchCandidates();
        } catch (error) {
